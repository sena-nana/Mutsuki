...
| `BatchPayload` / `PayloadLayout` | `layout + payload` 形式的执行面 payload descriptor；builtin 可持有仅进程内有效的 `LocalTaskPayload`，序列化时仍严格输出既有 Row wire shape；Columnar / BinaryPacked / ResourceBacked 是 SIMD-friendly 能力面 |
| `ProtocolClass` | load plan 中每个 task protocol 的显式运行时语义分类：Domain / Effect / Core / Control；前缀不是运行时分类依据 |
...
| `RunnerDescriptor` | runner_id、plugin_id、generation、accepted_protocol_ids、purity、execution_class、invocation_mode、concurrency、schema、batch/payload/resource/ordering/control capability、metadata |
| `ExecutionClass` | host 执行池分类：Control、Orchestration、Io、Cpu、Blocking、Script |
| `InvocationMode` | Host 调用边界：SyncExclusive、AsyncReentrant、AsyncExclusive、ExternalProcess |
| `RunnerConcurrency` | 逻辑 runner id 的并发声明：Exclusive、Reentrant（批次/entry 上限）、Sharded（固定实例数） |
| `AsyncInvocation` / `AsyncInvocationHandle` | Host 异步执行器使用的 invocation identity、task lease/attempt/generation fence、取消 token、deadline 与容量计数 |
...
| `CapabilityRequestEnvelope` / `DeliveryReceipt` / `IdempotentReceiptStore` / `ReceiptRetentionPolicy` | 传输无关的 typed capability 请求信封、回执与幂等重放；内存 store 支持 `max_entries` / `max_bytes` / TTL，过期 ID 视为未见过（TreatAsNew）；Core 不解释具体业务 payload，也不负责进程启动或本机端点路径 |
...
`RunnerDescriptor`、`PluginManifest` 和 `TaskAwaitRunnerAdapter` glue；它们不是新的 wire
...
- `RunnerDescriptor.batch` 声明 preferred / max batch entries、max inflight batches、
  ...
  reentrancy 和 side-effect 能力；`RunnerConcurrency` 是批次间并发权威，二者的
  `max_inflight_batches` 必须一致。`Exclusive` 固定为 1；`Reentrant` 显式给出批次与
  entry 上限；`Sharded { instances }` 要求 registry 在 freeze 前物化恰好相同数量、
  descriptor 完全一致的实例。Host limits、scheduler budget 与资源冲突只能进一步收紧。
...
Core 不轮询 Rust `Future`，也不提供 Promise、Coroutine、join/select、TaskGroup、WaitSet
或通用 executor。`AsyncBatchHandler` 只把 Host 可驱动的 Future 封装为 registry target；
真正的 poll、timer、wake、panic/timeout/cancel 与容量控制全部归 Host async executor。
最小 Host 未注入 executor 时必须以 `host.async_executor.unavailable` 结构化失败。

`TaskAwaitRunnerAdapter` 只接受 Mutsuki child-task awaitable，并降级为
`RunnerStatus::Waiting + TaskAwait`；它不是外部 I/O executor。Python mirror 同样只接受
Mutsuki task awaitable；普通 `asyncio` Future 或其他语言 awaitable 必须以
...
- Effectful runner 只能 claim `ProtocolClass::Effect` task。
- Committer runner 只能 claim `ProtocolClass::Core` 或 `ProtocolClass::Control` task。
- `ProtocolClass::Effect` 只能由 Effectful runner claim；`ProtocolClass::Core` / `Control`
  只能由相应 committer/control 边界处理。
- `ProtocolClass::Core` / `Control` 只能由 Committer runner claim；`core.*` 仅为规范命名与
  legacy import 的分类提示。
...
- host 必须将 `DispatchBudget.max_batches` clamp 到 `RunnerConcurrency`、Host limits、
  已有 active batch 和 executor/pool 可用 slot 的共同上限。
...
- `HostCapacity` 必须暴露 running / queued batch 与 entry 数、saturation、
  ...
  供 host scheduler 只读决策。Exclusive 已有 active batch 时 hard capacity 必须为 `0`；
  Reentrant/Sharded 则按声明和剩余容量计算。
...
- removed surface 的 zero occupancy 判定必须来自 TaskPool、ResourceManager 等当前
  事实源，而不是手动缓存。
- effect occupancy 来自 ready/running `ProtocolClass::Effect` task；stream occupancy 来自
