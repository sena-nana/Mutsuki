...
当前 core 已把 runner 调用预拆成 `RunnerDispatchTarget::{Sync, Async} -> Host executor ->
RunnerCompletion`。`InlineRunnerExecutor` 只执行同步 target，用于测试、replay 和最小
host；异步 target 在未注入 Host executor 时 fail-loud。默认 `HostRuntime` 启动
CoreActor 控制线程：Orchestration / Cpu 进入 compute pool，Io / Blocking / Script 进入
bounded blocking pool；显式 async invocation 则进入可注入的 `AsyncExecutor`（默认实现为
固定 worker 数的 Tokio runtime）。线程、Future poll 和 timer 都不进入 core crate；Core
只保留 claim、TaskLease/attempt/generation fencing 和 completion route 的事实源。
...
descriptor 一致性，scheduler 再按 Host limits、executor/pool 容量、payload bytes 与
资源冲突收紧 `DispatchBudget.max_batches`。一个 `WorkBatch` 内仍受
`max_entry_concurrency` 与 `WorkResourcePlan` 约束。
...
同步 `Io`、`Blocking` 和 `Script` 默认进入独立 blocking pool，避免阻塞调用、FFI、
外部进程或脚本解释器污染 CoreActor/compute 调度延迟；只有声明 async invocation mode
且注册 `AsyncBatchHandler` 的 runner 才进入 async executor lane。
...
这是 child-task await 语法糖；`TaskAwaitRunnerAdapter` 不承担外部 I/O Future。另一条
明确分离的 `AsyncBatchHandler` 边界可向 Host 返回 Rust Future，但 Core 从不 poll 它，
也不提供 language executor、`join_all`、`select`、`TaskGroup` 或 `WaitSet`。
...
- `HostContext` 聚合 active `CapabilityBroker`、host-only `HostServiceRegistry`、
  `ConfigProvider`、`EventBridge`、`TaskSubmitter`、同步 `ResourcePlanGateway`、可选
  `AsyncResourcePlanGateway` 和 `ShutdownController`。
- `TaskSubmitter` 和同步/异步 ResourcePlan gateway 只执行现有 `Task`、`TaskHandle`、
  `TaskOutcome` 与 resource plan 协议；builtin/ABI 等部署差异仍停留在 host backend。
