...
fn check_command(args: Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let remote: Box<dyn mutsuki_module_catalog::RemoteHeadProvider> = if fixture {
        build_fixture_remote(&release_set)
    } else {
        Box::new(mutsuki_module_catalog::ReqwestRemoteHeadProvider::default())
    };
...
fn build_fixture_remote(
    release_set: &mutsuki_module_catalog::ReleaseSetInfo,
) -> Box<dyn mutsuki_module_catalog::RemoteHeadProvider> {
...
mod cli_tests {
    use super::*;
    use mutsuki_module_catalog::UpgradeStatus;
