//! Link endpoint descriptors for Standalone-mode WebHost assembly.
//!
...
//! Product bots use Embedded Web Console; `quic://` is not a product endpoint.
...
/// Parse a `link_endpoint` string such as `local://mutsuki.servicehost`.
pub fn parse_link_endpoint(endpoint: &str) -> WebHostResult<LinkBridgeTarget> {
    ...
    match scheme {
        ...
        other => Err(WebHostError::InvalidConfig(format!(
            "unsupported link endpoint scheme `{other}` (supported: local)"
        ))),
    }
}
...
mod tests {
    ...
    #[test]
    fn rejects_quic_link_endpoint() {
        assert!(parse_link_endpoint("quic://127.0.0.1:4433").is_err());
    }
