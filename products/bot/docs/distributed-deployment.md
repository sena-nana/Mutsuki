...
The committed product configuration is explicit and inert:
...
`disabled` does not read a deployment file and the product starts no sidecar process, listener,
...
The schema-v2 deployment file must pin the same DistributedHost release and revision as the product,
...
safety, quality, cache and partial-result choices. The product checks that the referenced catalog is
