...
tests remain authoritative for implementation details; the product verifies immutable pins,
assembly boundaries and end-to-end deployment behavior without copying their implementations.

| Invariant | Owner evidence | Product evidence |
| --- | --- | --- |
...
| Sidecar/network failure does not stop local Core/ServiceHost. | DistributedHost external adapter boundary | Product never spawns/supervises sidecar; authenticated health monitoring marks disconnect/recovery, explicit Fast fallback is degraded, and Durable/Critical fallback is rejected. |
| Core hot paths do not run network, consensus or blocking telemetry. | Core #17-20 | Product depends on contracts only; distributed implementation is not linked into ServiceRuntime. |
...
| Remote benefit includes scheduling, transfer, prewarm, interference and risk. | DistributedHost scheduler profitability benchmark/tests | Product pins that scheduler revision and exposes bounded resource budgets. |
