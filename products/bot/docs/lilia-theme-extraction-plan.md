...
> 状态：历史方案与实施记录（2026-07）。其中仓库名称、pin 和独立模板验收描述保留当时事实，
> 不代表当前 `products/bot` 的依赖或发布方式。
...
## 附：Web Console 以 Lilia 重设计验收记录（2026-07-24）

在 theme 已 pin 的前提下，按依赖顺序 owner 先改先推、再 pin 模板，把嵌入式 Web Console
从「换了 token 的旧 Koishi 管理台」重设计为「以 Lilia 布局语言为准」的 Console。

| 仓库 | commit（已 push main） | 关键动作 |
| --- | --- | --- |
| MutsukiWebHost | `e4070a2511b9f88e303ba932816eaf5f6cd70e3f` | `@mutsuki/ui`：`console.css` 收敛为最小产品 chrome（不再重声明字体/base/表面），壳改用 Lilia `secondary-panel` + `sb-tree__row` 导航、`page-header` + base 表面主区；`consoleShell.ts`（Vue + vanilla factory）同步新壳；warn 走 `--warn`/`--warn-soft` 不复用 `--accent`；按钮/输入沿用 theme 稳定 32px 尺寸并补 focus-visible/hover/disabled。`pnpm --filter @mutsuki/ui build`（tsc+bundle）过，dist 41,580 bytes。 |
| MutsukiBotPlugins | `ccdc1ebc79f5cac7fad353d400e30f13a05e137a` | overview/config `index.js` 壳改为同一套 Lilia chrome；概览去 `.card--flat` 网格，用分区标题 + `.kv`/弱分割列表表达系统状态/指标/health/secret，degraded→warn；修 `flash()` 误引用 `.workspace`；导航 active 用 `sb-tree__row.is-active` + `data-lilia-selected`。重跑 `sync-mutsuki-ui-css.sh` 物化 3 份 `mutsuki-ui.css`（md5 一致，追溯到 `@mutsuki/ui e4070a2`）。web-console/overview/config `cargo test` 全过。 |
| MutsukiBotTemplate | 本次提交 | pin 5 个 BotPlugins 依赖 `b2da81c`→`ccdc1eb`；`cargo update` 刷新 lock；`cargo metadata --locked`、`fmt --check`、`cargo test -p mutsuki-bot`（含两个 web_console smoke）全过。WebHost Rust pin 仍 `f77cb0b`（重设计只动 `@mutsuki/ui` JS/CSS，已物化进 BotPlugins）。 |

### 设计对照（DESIGN §1–§7）与偏差

- **壳（§2/§6）**：Overview/Config 共用 Lilia Workspace chrome——elev 工作区表面 + `secondary-panel` 导航（`sb-tree__row` 稳定 28px 行，hover/active 走 state layer）+ base 表面主区 + 弱分割 `page-header`。
- **概览（§7）**：不再是卡片网格，内部 Region 用分区标题 + `kv`/列表 + 弱分割呈现，符合「内部 Region 不形成多张独立卡片」。
- **语义色（§6）**：degraded/缺失走 `--warn`/`--warn-soft`，危险操作仅在 hover/确认态染错误色。
- **样式来源（§4）**：只消费 `@lilia/theme`（经 `@mutsuki/ui` dist 物化）+ 最小 `console.css`；无内联大段 CSS 或旧分叉残留。
- **与「完整 Vue LiliaWorkspace」的差距**：Console 仍是 vanilla JS + Lilia 布局语言（theme class），未上 `@lilia/ui` 的 `LiliaWorkspace`/`LiliaSidebarFrame`/`LiliaSidebarRow` Vue 组件与受控 resize/折叠、Overlay 体系、Lucide 图标按钮。生产 WebHost 不跑 Node，若上 Vue 需走 WebHost shared runtime 预编译 ESM（externalize vue），属后续可选增量；本次交付先做到「视觉与信息架构就是 Lilia」。
