...
- Execute every matching Source chain independently on the one live graph. Multiple outgoing edges
  are explicit fan-out; multiple incoming edges trigger independently; version 1 has no cycles or
  implicit join.
...
- Persist Flow as the `mutsuki.bot.flow` provider document through `ConfigService`. Saving applies
  the graph once with document revision CAS and immediately activates it. Never add a Flow-specific
