...
use mutsuki_plugin_bot_agent::{AgentBridgeClient, BOT_AGENT_BRIDGE_PLUGIN_ID};
...
async fn service_runtime_routes_qq_event_through_agent_and_suppresses_replay() {
    ...
    assert_eq!(agent_inputs.lock().unwrap()[1], "command prompt");
    assert_eq!(syntheses.load(Ordering::SeqCst), 2);
    assert_eq!(qq_sends.load(Ordering::SeqCst), 4);
