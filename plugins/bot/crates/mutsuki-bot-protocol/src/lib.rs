...
/// Envelope `protocol_id` on Gateway-mapped events. Not a runner; adapters submit `flow/ingress@1`.
...
/// Envelope `protocol_id` on Command Match `matched` output. Not a runner protocol.
