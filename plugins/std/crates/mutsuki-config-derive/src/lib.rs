...
fn expand(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    for (index, field) in fields.named.iter().enumerate() {
        ...
        let visibility = match attr.visible_if {
            Some(expr) => {
                quote!(Some(::mutsuki_config_service::ConfigExpr::parse_simple(#expr).expect("visible_if")))
            }
            None => quote!(None),
        };
        let enabled_if = match attr.enabled_if {
            Some(expr) => {
                quote!(Some(::mutsuki_config_service::ConfigExpr::parse_simple(#expr).expect("enabled_if")))
            }
            None => quote!(None),
        };
        ...
        let description = match attr.description {
            Some(text) => quote!(Some(::mutsuki_config_service::LocalizedText::new(#text))),
            None => quote!(None),
        };

        children.push(quote! {
            ::mutsuki_config_service::ConfigNode {
                key: ::mutsuki_config_service::ConfigKey::new(stringify!(#name)),
                value_type: #ty_tokens,
                title: ::mutsuki_config_service::LocalizedText::new(#field_title),
                description: #description,
                default_value: #default_value,
                constraints: ::mutsuki_config_service::ConfigConstraints {
                    required: #required,
                    min: #min,
                    max: #max,
                ...
                presentation: ::mutsuki_config_service::ConfigPresentation {
                    group: #group,
                    order: #order,
                    unit: #unit,
                ...
                mutability: ::mutsuki_config_service::ConfigMutability::ReadWrite,
    ...
    Ok(quote! {
        impl ::mutsuki_config_service::MutsukiConfigSchema for #ident {
            fn schema() -> ::mutsuki_config_service::ConfigDescriptor {
                let descriptor = ::mutsuki_config_service::ConfigDescriptor {
                    provider_id: ::mutsuki_config_service::ConfigProviderId::new(#provider_id),
                    schema_version: #schema_version,
                    value_version: #value_version,
                    title: ::mutsuki_config_service::LocalizedText::new(#title),
                    description: None,
                    scopes: vec![::mutsuki_config_service::ConfigScope::plugin_instance()],
                    root: ::mutsuki_config_service::ConfigNode {
                        key: ::mutsuki_config_service::ConfigKey::new("root"),
                        value_type: ::mutsuki_config_service::ConfigValueType::Object,
                        title: ::mutsuki_config_service::LocalizedText::new(#title),
                        description: None,
                        default_value: None,
                        constraints: ::mutsuki_config_service::ConfigConstraints::default(),
                        presentation: ::mutsuki_config_service::ConfigPresentation::default(),
                        visibility: None,
                        enabled_if: None,
                        mutability: ::mutsuki_config_service::ConfigMutability::ReadWrite,
...
fn restart_tokens(value: Option<&str>) -> proc_macro2::TokenStream {
    match value {
        Some("none") | None => quote!(::mutsuki_config_service::RestartPolicy::None),
        Some("reconfigure") => quote!(::mutsuki_config_service::RestartPolicy::Reconfigure),
        Some("plugin_reload") => quote!(::mutsuki_config_service::RestartPolicy::PluginReload),
        Some("bot_restart") => quote!(::mutsuki_config_service::RestartPolicy::ApplicationRestart),
        Some("host_restart") => quote!(::mutsuki_config_service::RestartPolicy::HostRestart),
        Some(other) => {
...
fn type_to_value_type(ty: &syn::Type, attr: &FieldAttr) -> syn::Result<proc_macro2::TokenStream> {
    ...
    if attr.secret || ty_str == "SecretValue" {
        return Ok(quote!(::mutsuki_config_service::ConfigValueType::Secret));
    }
    if ty_str == "PathBuf" || ty_str == "std::path::PathBuf" {
        return Ok(quote!(::mutsuki_config_service::ConfigValueType::FileRef));
    }
    Ok(match ty_str.as_str() {
        "bool" => quote!(::mutsuki_config_service::ConfigValueType::Bool),
        "i8" | "i16" | "i32" | "i64" | "u8" | "u16" | "u32" | "u64" | "usize" | "isize" => {
            quote!(::mutsuki_config_service::ConfigValueType::Integer)
        }
        "f32" | "f64" => quote!(::mutsuki_config_service::ConfigValueType::Float),
        "String" => {
            let multiline = attr.multiline;
            quote!(::mutsuki_config_service::ConfigValueType::String { multiline: #multiline })
        }
        other if other.starts_with("Vec<") => {
            let inner = &other[4..other.len() - 1];
            let item = match inner {
                "String" => {
                    quote!(::mutsuki_config_service::ConfigValueType::String { multiline: false })
                }
                "bool" => quote!(::mutsuki_config_service::ConfigValueType::Bool),
                "u32" | "i32" | "u64" | "i64" | "usize" => {
                    quote!(::mutsuki_config_service::ConfigValueType::Integer)
                }
                _ => quote!(::mutsuki_config_service::ConfigValueType::String { multiline: false }),
            };
            quote!(::mutsuki_config_service::ConfigValueType::Array {
        ...
        other if other.starts_with("Option<") => {
            ...
            match inner {
                ...
                "PathBuf" | "std::path::PathBuf" => {
                    quote!(::mutsuki_config_service::ConfigValueType::FileRef)
                }
                _ => quote!(::mutsuki_config_service::ConfigValueType::String { multiline: false }),
            }
        }
...
fn expr_to_config_value(expr: &Expr) -> syn::Result<proc_macro2::TokenStream> {
    match expr {
        Expr::Lit(syn::ExprLit {
            ...
            let v = s.value();
            Ok(quote!(::mutsuki_config_service::ConfigValue::String(#v.to_string())))
        }
        Expr::Lit(syn::ExprLit {
            lit: Lit::Bool(b), ..
        }) => {
            let v = b.value;
            Ok(quote!(::mutsuki_config_service::ConfigValue::Bool(#v)))
        }
        Expr::Lit(syn::ExprLit {
            lit: Lit::Int(i), ..
        }) => {
            let v: i64 = i.base10_parse()?;
            Ok(quote!(::mutsuki_config_service::ConfigValue::Integer(#v)))
        }
        Expr::Lit(syn::ExprLit {
            lit: Lit::Float(f), ..
        }) => {
            let v: f64 = f.base10_parse()?;
            Ok(quote!(::mutsuki_config_service::ConfigValue::Float(#v)))
        }
