...
def runner_context(
    *,
    ...
    return RunnerContext(
        ...
        tick_id=TickId(tick_id),
        batch_id=BatchId(batch_id),
        executor_id=ExecutorId("executor:test"),
        task_lease_ids=tuple(TaskLeaseId(lease_id) for lease_id in lease_ids),
...
def single_test_batch(
    ...
    *,
    ...
    leased = replace(task, lease_id=TaskLeaseId(lease_id))
    lease = TaskLease(
        lease_id=TaskLeaseId(lease_id),
    ...
    return WorkBatch(
        ...
        entries=(
            BatchEntry(
                entry_id=EntryId(leased.task_id),
                task_id=leased.task_id,
...
def multi_entry_batch(
    ...
    *,
    ...
    leased_tasks = tuple(
        replace(task, lease_id=TaskLeaseId(lease_id))
        for task, lease_id in zip(tasks, lease_ids, strict=True)
    )
    entries = tuple(
        BatchEntry(
            entry_id=EntryId(task.task_id),
