...
| What implements behavior? | Replaceable implementation | `*Plugin` | `ExamplePlugin`, `MemoryPlugin` |
| What is a standard plugin pack? | Product-domain capability set | `*Core` | `LiliaCore`, `YumeCore` |
| How is an external API connected? | External translation or supplier | `*Adapter`, `*Provider` | `ExternalAdapter`, `ModelProvider` |
...
A bridge should convert shapes such as `RuntimeEvent -> Tauri event`, `ResourceRef -> blob id`, `EffectRequest -> pending interaction`, or `JSONL line -> RunnerResult`. If it chooses between implementations, it is a `Policy` or `Router`, not a bridge.
...
Use `Plugin` for replaceable behavior implementations, for example `ExamplePlugin` or `MemoryPlugin`.
...
Use `Adapter` for external or legacy translation, such as `AppServerAdapter` or `LegacyRunnerAdapter`.

Use `Provider` for concrete external capability suppliers, such as `ModelProvider`, `ExternalProvider`, `HttpProvider`, `BrowserProvider`, or a resource provider.
