...
var WebBridgeClient = class {
  constructor(url, options = {}) {
    ...
    this.helloTimeoutMs = positive(options.helloTimeoutMs, 5e3);
    this.requestTimeoutMs = positive(options.requestTimeoutMs, 3e4);
    this.maxPending = positive(options.maxPending, 128);
    this.reconnectBaseDelayMs = positive(options.reconnectBaseDelayMs, 1e3);
  ...
  helloTimer = null;
  idSequence = 0;
  async connect(protocolVersion = "1.0.0") {
  ...
  openGeneration(protocolVersion) {
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.addEventListener("message", (event) => {
        void decodeWireMessage(event.data).then((message) => {
          if (!this.isCurrent(generation, ws)) return;
  ...
  async call(namespace, method, params = null, options = {}) {
  ...
  read(namespace, method, params = null) {
    return this.call(namespace, method, params, { operation: "read" });
  }
  write(namespace, method, params = null) {
  ...
  handleDisconnect(generation, protocolVersion) {
  ...
  scheduleReconnect(protocolVersion) {
    ...
    this.reconnectTimer = setTimeout(() => {
      ...
      if (this.connectionState === "closed") return;
      this.connectPromise = this.openGeneration(protocolVersion);
      void this.connectPromise.catch(() => void 0);
    }, delay);
  }
  failHello(generation, error) {
  ...
  rejectGeneration(generation, reason) {
    for (const [id, pending] of this.pending) {
      if (pending.generation !== generation) continue;
      this.pending.delete(id);
      clearTimeout(pending.timer);
      const code = reason === "closed" ? "closed" : pending.sent && pending.operation === "write" ? "delivery_unknown" : "disconnected";
      pending.reject(
        new WebBridgeError(
          code,
          code === "delivery_unknown" ? "write may have been delivered before disconnect" : `web bridge ${reason}`
        )
      );
    }
  }
  restoreSubscriptions(generation) {
  ...
  sendCurrent(generation, payload) {
