...
use mutsuki_web_extension_control::{ControlRpcCaller, FixtureControlHandler};
use mutsuki_web_extension_overview::{OverviewWebExtension, materialize_frontend_assets};
...
async fn start(fail_statistics: bool) -> MutsukiWebHost {
    ...
    assert!(frontend.contains("overview-cards"));
    assert!(frontend.contains("overviewCardSpan"));
    assert!(frontend.contains("--overview-card-col-span"));
    assert!(frontend.contains("--overview-card-row-span"));
