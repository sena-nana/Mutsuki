...
use mutsuki_web_extension_control::CAPABILITY_RUNTIME_READ;
use mutsuki_web_protocol::{AssetEntry, ExtensionManifest, JsonValue, WebFrontendAssets};
...
fn filter_modules(
    modules: Vec<mutsuki_module_catalog::ModuleUpgradeSummary>,
    query: Option<&str>,
) -> Vec<mutsuki_module_catalog::ModuleUpgradeSummary> {
...
fn run_remote_check(
    release_set: ReleaseSetInfo,
    remote: Arc<dyn RemoteHeadProvider>,
) -> Result<Vec<mutsuki_module_catalog::ModuleUpgradeSummary>, ExtensionError> {
...
fn map_catalog_error(err: mutsuki_module_catalog::CatalogError) -> ExtensionError {
